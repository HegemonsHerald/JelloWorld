...
import static series12.SpeedRunner.GameState.*;

// package programming.set12.speedrunner;
...
public class SpeedRunner extends GraphicsProgram {
	...
	/**
	 * Initialization method. Add one time initializations, such as handler
	 * registrations, here. You are allowed to add code to this method.
	 */
	@Override
	public void init() {
		super.init();
	...
	/**
	 * The run method initialized the game and calls the main game loop. At the end
	 * the application's shutdown is forced. Do not change this method.
	 */
	@Override
	public void run() {
		gameInit();
	...
	/**
	 * {@link GameInit} initializes the game objects. Therefore, the player, the
	 * playing field and the status label objects are created. Additionally, the
	 * windows if adjusted to the appropriate size. Do not change this method.
	 */
	public void gameInit() {
		...
		// Set window size.
		setSize(HORIZONTAL_BLOCK_COUNT * BLOCK_SIZE + BOARD_OFFSET * 2,
				VERTICAL_BLOCK_COUNT * BLOCK_SIZE + BOARD_OFFSET * 2 + (int) status.getHeight());
	}

	/**
	 * The outer game loop switches between the different phases of the game until
	 * the {@link gameState} is set to EXITING. The phases are implemented in
	 * {@link GameWait}, {@link GameStart}, and {@link GamePlay}. Do not change this
	 * method.
	 */
	...
	/**
	 * {@link GameWait} is the first phase of the game. The loop idles and waits for
	 * the {@link gameState} to change. If the state is set to GENERATING, a new
	 * maze will be created. Otherwise, the loop is exited and the game proceeds
	 * according to state. Do not change this method.
	 */
	 ...
	 * Afterwards, a count down is displayed that waits for the specified start wait
	 * time. Do not change this method.
	 */
	public void gameStart() {
		// Set players starting position.
		player.setLocation(BLOCK_SIZE / 2 - PLAYER_SIZE / 2, BLOCK_SIZE / 2 - PLAYER_SIZE / 2 + board.getHeight() / 2);
		...
		do {
			...
			status.setLabel("Game will start in " + String.format("%.1f", Math.max(0, WAIT_TIME - elapsedTime) / 1000d)
					+ " seconds...");
			pause(100);
	...
	/**
	 * The inner game loop is iterated until the player reaches the right side of
	 * the playing board. Every frame, the {@link GamePlayFrame} method is called.
	 * Additionally, the status labels shows the elapsed time since game start. Do
	...
	public void gamePlay() {
		...
		status.setLabel(
				"Finished round in " + String.format("%.2f", seconds) + " seconds! Press SPACE to start again...");
	}

	/**
	 * {@link GamePlayFrame} is called every frame by the {@link GamePlay} method.
	 * Basically, this method is responsible for the player movement. Complete this
	 * method. You are allowed to change the present code.
	 */
	public void gamePlayFrame() {

		// possibly move in y direction
		if (moveUp) {
			double newY = player.getY() - 7.d; // I got one question: WHY 7? Why the fuck does this work?
			GObject gg = board.getElementAt(player.getX(), newY + PLAYER_SIZE);
			if (gg == null)
				player.setLocation(player.getX(), newY);
			moveUp = false;
		} else if (moveDown) {
			double newY = player.getY() + 7.d;
			GObject gg = board.getElementAt(player.getX(), newY + PLAYER_SIZE);
			if (gg == null)
				player.setLocation(player.getX(), newY);
			moveDown = false;
	...
	/**
	 * Creates a new, random playing field. GRects, which are not the player, are
	 * obstacles. You do not need to change this method. You can, of course, add own
	 * mazes for testing purposes.
	 *
	 * @param horizontalBlocks Number of horizontal blocks.
	 * @param verticalBlocks   Number of vertical blocks.
	 * @return Returns the playing field as GCompound.
	 */
	...
	/**
	 * Convenient method to create a new playing field. If a board is already
	 * present, remove it. Then, create a new board, offset its position, and add
	 * the player to the same compound. Also add the compound to the root pane. Do
	 * not change this method.
	 */
